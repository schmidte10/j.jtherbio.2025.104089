...
Understanding thermal perfomance curves of anaerobic and aerobic enzymes can help identify intrespecific differences in thermal tolerance. In this study four different species including, _Apogon rubrimacula_, _Apogon doederlein_, _Pomeacentrus australia_, and _Pomacentrus moluccensis_ had white muscle tissue samples collected, and used to determine thermal performance curves of two enzymes - lactate dehydrogenase (LDH) and citrate synthase (CS). Each enzyme was tested over five different temperatures including, 10$^\circ$C, 20$^\circ$C, 30$^\circ$C, 40$^\circ$C, and 50$^\circ$C, to create a thermal performance curve. Species in this study consisted for two cardinalfish and damselfish species, with different levels of reliance on coral reef structures. Results from this study will help determine interspecific differences in energetic metabolism in multiple coral reef species to help understand how different species may respond differently to changing environmental conditions. 
...
import.data3 <- read_delim("import_files/fish_mass_enzymes.txt", 
    delim = "\t", escape_double = FALSE, 
    col_types = cols(standard_length = col_number(), 
        mass = col_number()), trim_ws = TRUE) %>% 
  unite("fish_id", c("species","replicate"), remove=FALSE) %>% 
...
Data points have been checked using previously written app script that allows the user to look at the plotted points of samples that were run. Because we are interested in the slope, points that plateaued were removed from the analysis, as it signifies that the reaction ran out of 'fuel' to use.
...
Step4 will merge the data frames created with the mean slopes and the background slopes.
...
Step5 is where enzyme activity levels are calculated. Within this step background activity level is taken into account and subtracted from slopes where background activity was >5% or more of the sample slope. 
...
### LDH v MASS 
```{r ldh-initial-plots-6, warning=FALSE, message=FALSE}
...
aic.c=AICc(ldh.model.1, ldh.model.1.p2, ldh.model.1.p3, k=3) 
...
Looks like the model where temperature is treated as a second order polynomial (quadratic) is better than the linear and third order polynomial model. Lets see if adding the variables **tissue.mass** and/or **mass** improves the model. 

- *tissue.mass* refers to the mass of the tissue sample that was used. But note that samples were typically between 30 - 40 mg, so some control over this metric was done prior to the analysis. 
- *mass* refers to the mass of the fish the sample was taken from. 
...
The variables **tissue.mass and mass do not** significantly improve the model, therefore they will be left out of the model moving forward. 
...
aic.c=AICc(ldh.model.1.p2, ldh.glmm.1.p2.log,ldh.glmm.1.p2.sqrt,  k=3) 
...
## Important note 2
The Gamma distribution does seem to improve the model, definitely helps solve the heteroscadisity problem, however, there are down sides. The dispersion assumption within the DHARMa diagnositics is violated. Let's move on and see how the data transformation models perform. 

Next we will try transforming the data.
...
See *ANOVA* tab for values reported in manuscript.
...
```{r rest-pairwise-5.2, eval =FALSE, echo=FALSE}
### Means - f(temperature) [ratios]
ldh.sqrt.model.1.p2 %>% update(sqrt(LDH_ACTIVITY) ~ factor(temperature) * LATITUDE * LATIN_GENUS  + (1|fish_id)) %>% emmeans(pairwise ~ temperature*LATIN_GENUS*LATITUDE, type = "response")  %>% regrid(transform = "log") %>% pairs(by =c("temperature")) %>% summary(infer=FALSE)
```




```{r rest-pairwise-5, eval = FALSE, echo=FALSE}
...
```{r ldh-pairwise-6, eval = FALSE, echo=FALSE}
### Genus comparisons
...
```{r ldh-pairwise-genus, eval= FALSE, echo=FALSE}
### Genus [combined]
...
```{r rmr-effect-size, eval= FALSE, echo=FALSE}
### effect size-[emmeans]
...
```{r ldh-effect-size, eval = FALSE, echo=FALSE}
...
cs3.filtered <- cs3 |> 
  filter(!(UNIQUE_SAMPLE_ID %in% c(
                                   ...
                                   "Arub_04_10_1", "Arub_04_10_2", "Arub_04_10_3", 
...
Great! Now we have all the data points that we want to keep. However, the data needs to manipulated in a way that we can obtain and pull out slopes from the absorption readings, and then calculate enzyme activity based on these slopes. This will involve a number of steps.  
...
### CS v MASS [SPECIES]
```{r cs-initial-plots-6, warning=FALSE, message=FALSE}
...
From our initial plots we can all see that the relationship between enzyme activity and temperature is no linear, but rather may represent either a quadratic or cubic relationship. Let's start the model selection process by comparing different polynomial models.  
...
Looks like the model where temperature is treated as a second order polynomial (quadratic model) is better than the and third order polynomial model. Lets see if adding the variable **tissue.mass** and/or **mass** improves the model. 
...
aic.c=AICc(cs.model.1.p2, cs.model.1.p2b,cs.model.1.p2c, cs.model.1.p2d, k=5) 
...
The variable **tissue.mass and mass do not** improve the model, therefore they will be left out of the model moving forward. 
...
aic.c=AICc(cs.model.1.p2, cs.model.1.p2.log, cs.model.1.p2.sqrt,  k=3) 
...
```{r cs-pairwise-genus, eval = FALSE, echo=FALSE}
...
```{r cs-pairwise-genus2, eval = FALSE, echo=FALSE}
...
```{r cs-pairwise-genus3, eval = FALSE, echo=FALSE}
...
### Means - f(temperature)
```{r cs-pairwise-4, eval=TRUE}
cs.means <- cs.model.1.p2.log %>% update(CS_ACTIVITY ~ factor(temperature) * LATITUDE * LATIN_GENUS + (1|fish_id)) %>% emmeans(pairwise ~ temperature*LATIN_GENUS*LATITUDE, type = "response")

cs.means.df <- cs.means$emmeans %>% as.data.frame() %>% 
  mutate("change"=response - lag(response, n=1, default = first(response)), 
         "%change" = response/lag(response, n=1, default = first(response))*100) %>% 
  mutate("%change" = case_when(temperature == 10 ~ 0, 
                                     TRUE ~ `%change`), 
...
cs.means.df %>% knitr::kable(format = "html")  %>%
  kable_paper(full_width = TRUE) 
```


```{r cs-pairwise-5, eval = FALSE, echo=FALSE}
### Abs. diff - f(temperature)
...
```{r cs-pairwise-6, eval = FALSE, echo=FALSE}
### Genus comparisons
...
```{r cs-effect-size, eval = FALSE, echo=FALSE}
### effect size
...
From our initial plots we can all see that the relationship between enzyme activity (LDH:CS) and temperature is not linear, but rather may represent either a quadratic or cubic relationship. Let's start out model selection process by comparing different polynomial models.  
...
aic.c=AICc(lc.model.1, lc.model.1.p2, lc.model.1.p3, k=3) 
...
Looks like the model where temperature is treated as a second order polynomial (quadratic model) is better than the first and third order polynomial models. 

**tissue.mass and mass** did not improve the model when looking at either lactate dehydrogenase or citrate synthase, therefore we will not be including it in the LDH:CS ratio analysis.
...
aic.c=AICc(lc.model.1.p2, lc.model.1.p2.log, lc.model.1.p2.sqrt,  k=3) 
...
## Figure 1
```{r Figure-2, eval=FALSE, echo=TRUE, dpi=300}
#--- library ---#
library("magick") 
library("grid")
ruby <- image_read("./fish_figures/PNG/Ostorhinchus_rubrimacula.png")
ruby.vector <- magick::image_ggplot(ruby, interpolate = TRUE)  
ruby.image <- image_fill(ruby, 'none')
ruby.raster <- as.raster(ruby.image) 

doed <- image_read_svg("./fish_figures/SVG/Ostorhinchus_doederleini.svg")
doed.vector <- magick::image_ggplot(doed, interpolate = TRUE)  
doed.image <- image_fill(doed, 'none')
doed.raster <- as.raster(doed.image) 

australia <- image_read_svg("./fish_figures/SVG/paustralis2.svg")
australia.vector <- magick::image_ggplot(australia, interpolate = TRUE)  
australia.image <- image_fill(australia, 'none')
australia.raster <- as.raster(australia.image) 

molu <- image_read_svg("./fish_figures/SVG/lemon_damselfish.svg")
molu.vector <- magick::image_ggplot(molu, interpolate = TRUE)  
molu.image <- image_fill(molu, 'none')
molu.raster <- as.raster(molu.image)
#--- LDH plot ---# 
ldh.emm <- emmeans(ldh.sqrt.model.1.p2, ~temperature|LATITUDE|LATIN_GENUS, 
                   at = list(temperature = seq(from =10, to =50, by =.1)), 
                   type = "response") 
...
ldh.plot <- ggplot(ldh.emm.df, aes(y=response, x=temperature, linetype=species, color = species, fill=species))+
  ...
  geom_jitter(data=ldh.obs, 
              ...
              alpha = 0.3, 
              size = 2) +
  scale_x_continuous(limits = c(9, 56), 
                     breaks = seq(10, 52, by = 10))+ 
  scale_y_continuous(limits = c(0,350), breaks = seq(0, 350, by = 50)) +
  theme_classic() + 
  ylab(expression("LDH ACTIVITY (U mg "^{-1} * "tissue)")) + 
  ...
  annotation_raster(ruby.raster, 50, 56, 165, 215) + 
  annotation_raster(doed.raster, 50, 56, 290, 340) + 
  annotation_raster(australia.raster, 51, 56, 155, 205) + 
  annotation_raster(molu.raster, 50, 56, 200, 250)
...
cs.plot <- ggplot(cs.emm.df, aes(y=response, x=temperature, linetype=species, color = species, fill=species))+
  ...
  xlab("")+
  scale_linetype_manual(values = c("solid", "dashed","solid", "dashed"), 
                        labels = c("Ostorhinchus doederleini (sub-tropical)","Ostorhinchus rubrimacula (tropical)","Pomacentrus australis(sub-tropical)", "Pomacentrus moluccensis (tropical)"))+
  scale_color_manual(values=c("#3F3525","#C65C1B",  "#07AFF2", "#EAAE00"), 
                        labels = c("Ostorhinchus doederleini (sub-tropical)","Ostorhinchus rubrimacula (tropical)","Pomacentrus australis(sub-tropical)", "Pomacentrus moluccensis (tropical)")) +
  scale_fill_manual(values=c("#3F3525","#C65C1B",  "#07AFF2", "#EAAE00"), 
                        labels = c("Ostorhinchus doederleini (sub-tropical)","Ostorhinchus rubrimacula (tropical)","Pomacentrus australis(sub-tropical)", "Pomacentrus moluccensis (tropical)")) +  
  theme(legend.position = c(0.3,0.8), 
        ...
        axis.text = element_text(size=12), 
        axis.text.x = element_blank()) + 
  annotation_raster(ruby.raster, 50, 56, 4.5, 7) + 
  annotation_raster(doed.raster, 51, 57, 4.3, 6.8) + 
  annotation_raster(australia.raster, 50.5, 55, 7.5, 10) + 
  annotation_raster(molu.raster, 50, 56, 10, 12.5) 
...
lc.plot <- ggplot(lc.emm.df, aes(y=response, x=temperature, linetype=species, color = species, fill=species))+
  ...
  annotation_raster(ruby.raster, 50, 56, 30, 48) + 
  annotation_raster(doed.raster, 50, 56, 65, 83) + 
  annotation_raster(australia.raster, 50.5, 55, 18, 35) + 
  annotation_raster(molu.raster, 51, 57, 16, 33)
...
ggsave(file="./02.Figures/Figure1/Figure1_species_plot.png",  plot=species.plot, width=8.5, height=15.5, units = "in", dpi=300) 
ggsave(file="./02.Figures/Figure1/Figure1_species_plot2.png", plot=species.plot, width=7, height=12, units = "in", dpi=300)

ggsave(file="./02.Figures/Figure1/Figure1_species_plot.pdf",  plot=species.plot, width=8.5, height=15.5, units = "in", dpi=300)
ggsave(file="./02.Figures/Figure1/Figure1_species_plot2.pdf", plot=species.plot, width=7, height=12, units = "in", dpi=300)

ggsave(file="./02.Figures/Figure1/Figure1_species_plot.svg",  plot=species.plot, width=8.5, height=15.5, units = "in", dpi=300)
ggsave(file="./02.Figures/Figure1/Figure1_species_plot2.svg", plot=species.plot, width=7, height=12, units = "in", dpi=300)

```

## Supplemental Figure 1

```{r SFigure-1, eval=FALSE, echo=TRUE}
#--- LDH plot ---#
...
```{r Figure-1, eval=FALSE, echo=TRUE}
...
ggsave(file="./02.Figures/SFigure2/SFigure2_cs_genus_plot.svg",  plot=cs.genus.plot, width=15.5, height=8.5, units = "in", dpi=300) 
ggsave(file="./02.Figures/SFigure2/SFigure2_cs_genus_plot2.svg", plot=cs.genus.plot, width=12, height=7, units = "in", dpi=300)
ggsave(file="./02.Figures/SFigure2/SFigure2_cs_genus_plot3.svg", plot=cs.genus.plot, width=8, height=5, units = "in", dpi=300)
ggsave(file="./02.Figures/SFigure2/SFigure2_cs_genus_plot.pdf",  plot=cs.genus.plot, width=15.5, height=8.5, units = "in", dpi=300)
ggsave(file="./02.Figures/SFigure2/SFigure2_cs_genus_plot2.pdf", plot=cs.genus.plot, width=12, height=7, units = "in", dpi=300)
ggsave(file="./02.Figures/SFigure2/SFigure2_cs_genus_plot3.pdf", plot=cs.genus.plot, width=8, height=5, units = "in", dpi=300)
```

## Supplemental Figure x

```{r SFigure-2, eval=FALSE, echo=TRUE}
