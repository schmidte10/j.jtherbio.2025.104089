...
ldh.model.1.p2b <- glm(LDH_ACTIVITY ~ poly(temperature, 2) * LATITUDE * LATIN_GENUS + TISSUE_MASS_CENTERED, 
               family = gaussian(),
               data = ldh.data) 
```
...
ldh.glmm.1.p2 <- glmmTMB(LDH_ACTIVITY ~ poly(temperature, 2) * LATITUDE * LATIN_GENUS + (1|fish_id), 
               family = gaussian(), #default is link="identity"
               REML = FALSE,
               data = ldh.data) 

ldh.glmm.1.p2.log <- glmmTMB(LDH_ACTIVITY ~ poly(temperature, 2) * LATITUDE * LATIN_GENUS + (1|fish_id), 
               family = gaussian(link="log"),
               REML = FALSE,
               data = ldh.data)  

ldh.glmm.1.p2.sqrt <- glmmTMB(LDH_ACTIVITY ~ poly(temperature, 2) * LATITUDE * LATIN_GENUS + (1|fish_id), 
...
anova(ldh.model.1.p2, ldh.glmm.1.p2.log,ldh.glmm.1.p2.sqrt, test = "Chisq")
```
